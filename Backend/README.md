...
A FastAPI-based backend service for the NYC Smart City Dashboard, providing real-time traffic, transit, air quality monitoring, and ML-powered congestion predictions.

## 🚀 Features

- **Real-time Data Ingestion**: Automated ingestion from NYC DOT OpenData, MTA GTFS-Realtime, and Air Quality APIs
- **Data Processing**: Multi-agent system for ingestion, cleaning, correlation, and prediction
- **ML Predictions**: Machine learning models for traffic congestion forecasting (15min and 30min windows)
- **RESTful API**: FastAPI-based REST API with automatic OpenAPI documentation
...
├── app/
│   ├── agents/              # Multi-agent system
│   │   ├── agent1_ingestion.py      # Data ingestion agent
│   │   ├── agent2_cleaning.py       # Data cleaning & correlation
│   │   └── agent3_prediction.py     # ML prediction agent
│   ├── api/
│   │   └── routes/          # API endpoints
│   │       ├── segments.py # Traffic segments
│   │       ├── zones.py     # Traffic zones
│   │       ├── predictions.py # ML predictions
│   │       ├── health.py    # Health checks
│   │       └── explain.py  # AI explanations
│   ├── clients/             # External API clients
│   │   ├── traffic_dot.py   # NYC DOT OpenData
│   │   ├── traffic_511.py  # 511NY API
│   │   ├── transit_mta.py   # MTA GTFS-Realtime
│   │   └── air_quality.py   # Air quality APIs
│   ├── ml/                  # Machine learning
...
│   └── main.py              # FastAPI app
├── scripts/                 # Utility scripts
│   ├── run_simulation.py    # Populate test data
...
The backend uses a three-agent architecture:
...
This populates the database with mock traffic data for testing.
...
```bash
python scripts/verify_data_simple.py
```
...
### NYC DOT OpenData (Public, No API Key)
- Traffic Speeds (NBE)
- Traffic Volume Counts
- Motor Vehicle Collisions
- Bus Breakdowns and Delays
...
2. **Verify connection string**: Check `MONGODB_URI` in `.env`
3. **Check network access**: For Atlas, ensure IP is whitelisted
4. **Check authentication**: Verify username/password in connection string
